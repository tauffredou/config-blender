...
| `internal/centralapi`/`centralserver`/`centralclient` + `cmd/server` | Central service (Recipe DB + Git-source registry + Starlark), public read + role-gated write via per-account RBAC (`internal/userdb`: human session login or service-account API key) | Done, tested (including a real network round trip in-cluster) |
