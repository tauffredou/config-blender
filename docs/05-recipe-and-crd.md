...
**Repository authentication (implemented)**: `gitsource.AuthResolver` (a `repoURL -> transport.AuthMethod` function) is now concrete — `gitsource.BasicAuthMethod` (HTTP + token, e.g. GitHub PAT) and `gitsource.SSHAuthMethod` (PEM private key) build the credentials, `gitsource.NewAuthMap` allows one credential per repo with a default fallback. The `internal/gitauth` package feeds them from the process environment (`GIT_USERNAME`/`GIT_PASSWORD`/`GIT_TOKEN`, or `GIT_SSH_KEY`/`GIT_SSH_KEY_FILE`), used identically by the controller (`cmd/manager`) and the CLI (`internal/cli`). Consistent with 4.1: these are configblender's own operational credentials (repo access), not application secrets — they arrive through the standard K8s mechanism (a Secret mounted as env vars or a file), with no Secret-reading code specific to configblender to write. Accepted limitation for the MVP: a single credential set for all repos (no per-repo resolution from the environment — possible via `NewAuthMap` directly if needed, but not wired automatically).
...
- `configblender explain (--db <path> | --central-url <url>) --recipe <name> [--key <dotted.path>]` — prints the full explain, or a single key's provenance ([02-resolution-model.md §2.3](02-resolution-model.md)).

On the central-service side, these same reads are exposed over HTTP (`internal/centralserver`): `GET /v1/resolve?recipe=<name>`, `GET /v1/recipes` (list), `GET /v1/recipes/{name}` (latest version), `GET /v1/recipes/{name}/versions` (history), `GET /v1/recipes/{name}/versions/{n}` (a specific version) — no authentication.
